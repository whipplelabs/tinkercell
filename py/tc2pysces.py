def load():
      ...
      for i in range(0,n):
        ...
        for j in N[0]:
           if fixed[0][i] == j:
              ...
              break;
        if not inUse:
           for j in rates:   #check if used in rates
              if j.count(fixed[0][i]) > 0:
                 inUse = True;
              if inUse:
                 break;
        if not inUse:
           for j in funcsAssign:  #check if used in functions
              if j.count(fixed[0][i]) > 0:
                 inUse = True;
              if inUse:
                 break;
        if not inUse:
           for j in events:  #check if used in events
              if j.count(fixed[0][i]) > 0:
                 inUse = True;
              if inUse:
                 break;
  ...
  if len(params) == 2 and len(params[0]) == len(params[1]):
      ...
      for i in range(0,n):  #for each parameter
        ...
        #      if inUse:
        #         break;
        #if inUse:       #only if used, include in model
