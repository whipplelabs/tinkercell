...
class PysMod:
    """
    This is the PySCeS Core class, a.k.a. pysces.model()
    """
    ...
    def Stoichiometry_Analyse(self,override=0,load=0):
        ...
        self.Nmatrix = StructMatrix(self.__structural__.nmatrix, self.__structural__.nmatrix_row, self.__structural__.nmatrix_col)
        self.Nmatrix.setRow(self.species)
        self.Nmatrix.setCol(self.reactions)

        self.Nrmatrix = StructMatrix(self.__structural__.nrmatrix, self.__structural__.nrmatrix_row, self.__structural__.nrmatrix_col)
        self.Nrmatrix.setRow(self.species)
        self.Nrmatrix.setCol(self.reactions)

        self.Kmatrix = StructMatrix(self.__structural__.kmatrix, self.__structural__.kmatrix_row, self.__structural__.kmatrix_col)
        self.Kmatrix.setRow(self.reactions)
        self.Kmatrix.setCol(self.reactions)

        self.K0matrix = StructMatrix(self.__structural__.kzeromatrix, self.__structural__.kzeromatrix_row, self.__structural__.kzeromatrix_col)
        self.K0matrix.setRow(self.reactions)
        self.K0matrix.setCol(self.reactions)

        self.Lmatrix = StructMatrix(self.__structural__.lmatrix, self.__structural__.lmatrix_row, self.__structural__.lmatrix_col)
        self.Lmatrix.setRow(self.species)
        self.Lmatrix.setCol(self.species)

        self.L0matrix = StructMatrix(self.__structural__.lzeromatrix, self.__structural__.lzeromatrix_row, self.__structural__.lzeromatrix_col)
        self.L0matrix.setRow(self.species)
        self.L0matrix.setCol(self.species)

        if self.__structural__.info_moiety_conserve:
            self.Consmatrix = StructMatrix(self.__structural__.conservation_matrix, self.__structural__.conservation_matrix_row, self.__structural__.conservation_matrix_col)
...
#pysces core - the steady-state solver and integration routines
    ...
    def CVODE(self, initial):
        ...
        y = cvode.NVector(initial.tolist())
        cvode_mem = cvode.CVodeCreate(cvode.CV_BDF, cvode.CV_NEWTON) #initialisation with basic options, newtonian solver etc...
        ...
        cvode_scale_range = range(sim_st, len(self.sim_time), len(self.sim_time)/4)
        ...
        abstol = cvode.NVector(len(initial)*[self.__settings__["cvode_abstol"]])
        for s in range(len(y)):
            newVal = abs(y[s])*ABSTOL_ADJUST_FACTOR
            if newVal < MIN_ABS_TOL:
                abstol[s] = MIN_ABS_TOL
            else:
                abstol[s] = newVal
        cvode.CVodeMalloc(cvode_mem, func, 0.0, y, cvode.CV_SV, reltol, abstol) #set tolerances, specify vector of initial conditions, set integrtion function etc...
        cvode.CVDense(cvode_mem, len(initial)) #set dense option, specify dimension of problem (3)
        if self.__HAS_EVENTS__:
            cvode.CVodeRootInit(cvode_mem, len(self.__events__), self.CVODE_EVENTS, None) #specify to 'root' conditions, and function that calculates them
        del initial
        ...
        for st in cvode_sim_range:
            ...
            if self.__settings__["cvode_auto_tol_adjust"] and TOL_ADJUSTER >= MAX_TOL_CNT and reltol.value < MAX_REL_TOLERANCE:
                ...
                cvode.CVodeSetTolerances(cvode_mem, cvode.CV_SV, reltol, abstol)
            ...
            if (st in cvode_scale_range) and self.__settings__["cvode_auto_tol_adjust"]:
                ...
                cvode.CVodeSetTolerances(cvode_mem, cvode.CV_SV, reltol, abstol)
                ##  cvode.CVodeReInit(cvode_mem, func, self.sim_time[st-1], y, cvode.CV_SV, reltol, abstol)
            while True:
                try:
                    flag = cvode.CVode(cvode_mem, tout, y, cvode.ctypes.byref(t), cvode.CV_NORMAL)
                ...
                if flag == cvode.CV_ROOT_RETURN: #if a root was found before desired time point, output it
                    ya = numpy.array(y)
                    rootsfound = cvode.CVodeGetRootInfo(cvode_mem, len(self.__events__))
                    reInit = False
                        ...
                        if rootsfound[ev] == 1:
                            for ass in self.__events__[ev].assignments:
                                ...
                                (self.mode_integrate_all_odes and ass.variable in self.__species__):
                                    ...
                                    if self.__KeyWords__['Species_In_Conc']:
                                        ##  print y
                                        y[assIdx] = assVal*getattr(self, self.__CsizeAllIdx__[assIdx])
                                        ##  raw_input(y)
                                    else:
                                        y[assIdx] = assVal
                                    reInit = True
                                    ...
                                    ##  print self.L0matrix.shape[1], rrIdx, len(y)
                                    y[self.L0matrix.shape[1]+rrIdx] = assVal
                                    setattr(self, ass.variable, assVal)
                                    reInit = True

                    if reInit:
                        cvode.CVodeReInit(cvode_mem, func, tout, y, cvode.CV_SV, reltol, abstol)
                ...
                if flag == cvode.CV_SUCCESS:
                    ...
                    cvode.CVodeSetMaxNumSteps(cvode_mem, self.__settings__["cvode_mxstep"])
                elif flag < -3:
    ...
    def Simulate(self,userinit=0):
        ...
        - 2  initialise with results of previously calculated stead state
...
##                        File.write('  <td nowrap>' + self.__settings__['write_array_html_format'] % input[x,y] + '</td>\n')
    ...
    def Scan2DPlot(self, title=None, log=None, format='lines', filename=None):
        ...
        plt.splot(self.__scan2d_results__, 0, 1, 2, title=self.__scan2d_pars__[2], format=format)
        plt.setRange('xyz')
