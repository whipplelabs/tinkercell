...
class Scanner(object):
    ...
    def addScanParameter(self,name,start,end,points,log=False,slave=False):
        ...
        if not len(self.GenOrder) == 0:
            for el in self.GenOrder:
                if not slave:
                    offset = offset * self.GenDict[el][2]
                else:
                    points = self.GenDict[el][2]
                    log = self.GenDict[el][3]
                    offset = self.GenDict[el][5]
        else:
            if slave == True:
                slave = False
    ...
    def getResultMatrix(self, stst=False):
        """
        ...
        """
        if stst:
            return scipy.hstack([self.ScanSpace, self.SteadyStateResults, self.UserOutputResults])
        else:
            return scipy.hstack([self.ScanSpace, self.UserOutputResults])
