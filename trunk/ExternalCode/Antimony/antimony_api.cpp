...
LIB_EXTERN char* getCharStar(const char* orig)
{
...
LIB_EXTERN char** getCharStarStar(size_t size)
{
...
LIB_EXTERN char*** getCharStarStarStar(size_t size)
{
  char*** ret = (char***) malloc(size*sizeof(char**));
  if (ret == NULL) {
    g_registry.SetError("Out of memory error.");
...
LIB_EXTERN double* getDoubleStar(size_t size)
{
...
LIB_EXTERN double** getDoubleStarStar(size_t size)
{
  double** ret = (double**) malloc(size*sizeof(double*));
  if (ret == NULL) {
    g_registry.SetError("Out of memory error.");
...
LIB_EXTERN rd_type* getRDTypeStar(size_t size)
{
...
LIB_EXTERN size_t* getSizeTStar(size_t size)
{
...
LIB_EXTERN void reportReactionIndexProblem(size_t n, size_t actualsize, const char* moduleName, bool reaction=true)
{
...
LIB_EXTERN void reportVariableTypeIndexProblem(size_t n, return_type rtype, size_t actualsize, const char* moduleName)
{
...
LIB_EXTERN char** getNthRxnOrIntReactantOrProductNames(const char* moduleName, size_t n, bool reaction, bool reactant)
{
  if (!checkModule(moduleName)) return NULL;
  return_type rtype = allReactions;
  if (!reaction) {
...
LIB_EXTERN char*** getReactantOrProductNamesForRxnOrInt(const char* moduleName, bool reaction, bool reactant)
{
  if (!checkModule(moduleName)) return NULL;
  return_type rtype = allReactions;
  if (!reaction) {
...
LIB_EXTERN double* getNthRxnOrIntReactantOrProductStoichiometries(const char* moduleName, size_t n, bool reaction, bool reactant)
{
  if (!checkModule(moduleName)) return NULL;
  return_type rtype = allReactions;
  if (!reaction) {
...
LIB_EXTERN double** getReactantOrProductStoichiometriesForRxnOrInt(const char* moduleName, bool reaction, bool reactant)
{
  if (!checkModule(moduleName)) return NULL;
  return_type rtype = allReactions;
  if (!reaction) {
...
LIB_EXTERN size_t getSizeOfNthDNAStrand(const char* moduleName, size_t n)
{
  ...
  if (actualsize <= n) {
	char s[10];
	itoa(n,s,10);
    string error = string("There is no DNA strand with index ") + string(s);
    error += " in module ";
    error += moduleName;
    error += ".";
    if (actualsize == 0) {
...
LIB_EXTERN char** getNthDNAStrand(const char* moduleName, size_t n)
{
  ...
  if (actualsize <= n) {
	  char s[10];
	itoa(n,s,10);
    string error = "There is no DNA strand with index " + string(s);
    error += " in module ";
    error += moduleName;
    error += ".";
    if (actualsize == 0) {
...
LIB_EXTERN size_t getSizeOfNthModularDNAStrand(const char* moduleName, size_t n)
{
  ...
  if (actualsize <= n) {
	char s[10];
	itoa(n,s,10);
    string error = "There is no Modular DNA strand with index " + string(s);
    error += " in module ";
    error += moduleName;
    error += ".";
    if (actualsize == 0) {
...
LIB_EXTERN char** getNthModularDNAStrand(const char* moduleName, size_t n)
{
  ...
  if (actualsize <= n) {
	char s[10];
	itoa(n,s,10);
    string error = string("There is no Modular DNA strand with index ") + s;
    error += " in module ";
    error += moduleName;
    error += ".";
    if (actualsize == 0) {
