...
namespace Tinkercell
{
    ...
    void StoichiometryTool::deselect()
    {
    ...
    void StoichiometryTool::pluginLoaded(const QString&)
    {
...
// //         }
//
//         if (!connected2 && mainWindow->tools.contains(tr("Connection Selection")))
//         {
...
//         }
    ...
    void StoichiometryTool::setStoichiometrySlot(QSemaphore * s, QList<ItemHandle*>& items, const DataTable<qreal>& table)
    {
        ...
        {
            setStoichiometry(mainWindow->currentScene(),items,table);
        }
        if (s)
            s->release();
    }
    ...
    {
        if (mainWindow && mainWindow->currentScene())
            setRates(mainWindow->currentScene(),items,rates);
        if (s)
            s->release();
    }
    ...
    void StoichiometryTool::itemsInserted(GraphicsScene* scene, const QList<QGraphicsItem *>& , const QList<ItemHandle*>& handles)
    {
        for (int i=0; i < handles.size(); ++i)
        {
            if (handles[i] && handles[i]->type == ConnectionHandle::Type)
            {
                ...
                {
                    insertDataMatrix(scene,connectionHandle);
                }
            }
        }
        //qDebug() << "insertion done: " << this->name;
    }
    ...
    void StoichiometryTool::setStoichiometry(int , int )
    {
        ...
        if (mainWindow != 0 && mainWindow->currentScene() != 0)
            mainWindow->currentScene()->changeData(handles,tr("Stoichiometry"),nDataTablesNew);
    ...
    void StoichiometryTool::setRate(int , int )
    {
        ...
        if (mainWindow != 0 && mainWindow->currentScene() != 0)
            mainWindow->currentScene()->changeData(handles,tr("Rates"),sDataTablesNew);
        ...
        autoReverse("Make reversible",this),separator(0)
    {
        ...
        graphicsItem->addToGroup(&graphics1);
        graphicsItem->setToolTip(tr("Reaction rates"));
    ...
    void StoichiometryTool::insertDataMatrix(GraphicsScene * scene, ConnectionHandle * handle)
    {
        ...
        scenePtr += scene;
        numericalDataPtr += &(handle->data->numericalData[tr("Stoichiometry")]);
        textDataPtr += &(handle->data->textData[tr("Rates")]);
...
*/
    ...
    void StoichiometryTool::addRow()
    {
            ...
            lastItem->hasTextData(tr("Rates")))
        {
            ...
            if (mainWindow && mainWindow->currentScene())
            {
                mainWindow->currentScene()->changeData(QList<ItemHandle*>() << lastItem,nDat,nDataTable,sDat,sDataTable);
            }
    ...
    void StoichiometryTool::removeRow()
    {
        ...
        if (nDataTablesNew.size() > 0)
        {
            if (mainWindow != 0 && mainWindow->currentScene() != 0)
            {
                mainWindow->currentScene()->changeData(QList<ItemHandle*>() << handles,nDataTablesOld,nDataTablesNew,sDataTablesOld,sDataTablesNew);
            }
    ...
    void StoichiometryTool::setStoichiometry(GraphicsScene* scene, QList<ItemHandle*>& connectionHandles,const DataTable<qreal>& N, const QString& replaceDot)
    {
        ...
        for (int i=0; i < connectionHandles.size(); ++i) //build combined matrix for all selected reactions
        {
            if (connectionHandles[i] != 0 && connectionHandles[i]->data != 0)
            {
                if (connectionHandles[i]->hasNumericalData(QObject::tr("Stoichiometry")))
                {
                    ...
                    {
                        //sDataTable->rowName(k) = nDataTable->rowName(k) = stoicMatrix.rowName(n); //set row name
                        //if (sDataTable->value(k,0).isEmpty())
                        //sDataTable->value(k,0) = QString("0");
        ...
        if (scene)
        {
            scene->changeData(handles,QObject::tr("Stoichiometry"),nDataTablesNew);
        }
    ...
    void StoichiometryTool::setRates(GraphicsScene * scene, QList<ItemHandle*>& connectionHandles,const QStringList& list, const QString& replaceDot)
    {
        ...
        if (scene)
        {
            //scene->changeData(handles,thisName,nDataTablesNew,sDataTablesNew);
            scene->changeData(handles,QObject::tr("Rates"),sDataTablesNew);
        }
