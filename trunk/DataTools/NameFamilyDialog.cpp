...
namespace Tinkercell
{
	NameFamilyDialog::NameFamilyDialog() : Tool(tr("Name and Family Dialog"))
        {
            ...
            graphicsItem->addToGroup(&idcard);
            graphicsItem->setToolTip(tr("Name, family, and Annotation"));
	}
	...
	bool NameFamilyDialog::setMainWindow(MainWindow * main)
	{
        ...
        void NameFamilyDialog::itemsInsertedSlot(GraphicsScene * scene, const QList<QGraphicsItem*>& items, const QList<ItemHandle*>& handles)
	{
		if (!scene || items.isEmpty() || handles.isEmpty()) return;
	...
	void NameFamilyDialog::showDialog(const QList<ItemHandle*>& list)
	{
		selectedItems = list;

		QString oldName;//, oldFamilyName;
		bool unequalNames = false;
		...
		for (int i=0; i < list.size(); ++i)
		{
			...
			if (handle && handle->family())
			{
				...
				if (oldName.isEmpty() && !unequalNames)
					oldName = handle->name;
				else
					if (!unequalNames && oldName != handle->name)
					{
						unequalNames = true;
						oldName = tr("");
					}

				//if (oldFamilyName.isEmpty())
				//	oldFamilyName = handle->family()->name;
				if (!superfamily)
				{
					superfamily = handle->family();
				}
				else
				{
					if (superfamily->isA(handle->family()))
						superfamily = handle->family();
					else
						if (handle->family()->isA(superfamily))
						{
							//QMessageBox::information(mainWindow,this->name,tr("Please select one item or items of the same family."),QMessageBox::Ok,QMessageBox::Ok);
							OutputWindow::error(tr("Please select one item or items of the same family."));
							return;
						}
				}
			}
		...
		if (superfamily && lineEdits.size() > 1 && lineEdits[0] && lineEdits[1])
		{
			lineEdits[0]->setText(oldName);
			lineEdits[0]->setVisible(!unequalNames);
			lineEdits[1]->setText(superfamily->name);
	...
	{
                if (lineEdits.size() < 5 || !mainWindow || !mainWindow->currentWindow()) return;
		...
		{
			ItemHandle * handle = (selectedItems[0]);
			if (handle)
			{
		...
		for (int i=0; i < selectedItems.size(); ++i)
			if (selectedItems[i] && selectedItems[i]->type == ConnectionHandle::Type)
				containsConnections = true;

		QList<ItemHandle*> visitedHandles;

		if (family.isEmpty()) return;

// 		if (!mainWindow->tools.contains("Nodes Tree"))
// 		{
// 			//QMessageBox::information(mainWindow,this->name,tr("No nodes family tree available"),QMessageBox::Ok,QMessageBox::Ok);
// 			OutputWindow::error(tr("No nodes family tree available."));
// 			return;
// 		}

// 		NodesTree * nodesTree = static_cast<NodesTree*>(mainWindow->tools["Nodes Tree"]);
...
// 		}
		...
		if (!family.isEmpty())
		{
			...
			for (int i=0; i < selectedItems.size(); ++i)
                                if (NodeHandle::asNode(selectedItems[i]))
				{
					ItemHandle * handle = (selectedItems[i]);
					if (handle && handle->family() && !visitedHandles.contains(handle) && handle->family()->name != family)
					{
						visitedHandles << handle;

						QString oldFamily = handle->family()->name;

                                                NodeHandle * node = NodeHandle::asNode(selectedItems[i]);
                                                if (node && node == handle)
						{
							if (node->connections().isEmpty())// || nodesTree->nodeFamilies[familyName]->isA(oldFamily))
							{
...
{
								//QMessageBox::information(mainWindow,this->name,tr("Cannot change family of connected items"),QMessageBox::Ok,QMessageBox::Ok);
						...
						{
							/*ConnectionGraphicsItem * connection = qgraphicsitem_cast<ConnectionGraphicsItem*>(selectedItems[i]);
							if (connection)
							{
								QMessageBox::information(mainWindow,this->name,tr("Cannot change family of connections"),QMessageBox::Ok,QMessageBox::Ok);
								return;
							}*/
						}
					}
					//QMessageBox::information(mainWindow,this->name,tr("Cannot edit family information for this item"),QMessageBox::Ok,QMessageBox::Ok);
				}
			if (!commands.isEmpty())
                        {
	...
	void NameFamilyDialog::select()
	{
            NetworkWindow * win = currentWindow();
            if (win)
                showDialog(win->allHandles());
	}

        void NameFamilyDialog::deselect()
	{
                if (parentWidget() != 0)
                        parentWidget()->hide();
                else
