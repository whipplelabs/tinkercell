...
{
    void AssignmentFunctionsTool::select()
    {
    ...
    void AssignmentFunctionsTool::deselect()
    {
...
//         if (!connected && mainWindow && mainWindow->tools.contains(tr("Model Summary")))
//         {
...
//         }
    ...
    void AssignmentFunctionsTool::itemsInserted(GraphicsScene* scene, const QList<QGraphicsItem *>& , const QList<ItemHandle*>& handles)
    {
    ...
    void AssignmentFunctionsTool::insertDataMatrix(GraphicsScene * scene, ItemHandle * handle)
    {
        ...
        if (!handle->hasNumericalData(tr("Assignments")))
        {
        ...
        if (!handle->hasNumericalData(tr("Functions")))
        {
            DataTable<QString> table;
            table.resize(0,2);
