...
namespace Tinkercell
{
    ...
    void ModelSummaryTool::deselect()
    {
    ...
    void ModelSummaryTool::itemsInserted(GraphicsScene* scene, const QList<QGraphicsItem *>& , const QList<ItemHandle*>& handles)
    {
    ...
    ModelSummaryTool::ModelSummaryTool() : Tool(tr("Model Summary")), delegate(QStringList() << "floating" << "fixed")
    {
        ...
        graphicsItem->setToolTip(tr("Model summary"));
    }
    /*
        void ModelSummaryTool::pluginLoaded(const QString&)
        {
                static bool connected = false;
                if (connected) return;

                if (!connected && mainWindow && mainWindow->tools.contains(tr("Graph String Tool")))
                {
                        QWidget * widget = mainWindow->tools[tr("Graph String Tool")];
                        GraphStringTool * graphTool = static_cast<GraphStringTool*>(widget);

                        connect(this,SIGNAL(setValues(const QStringList&, const QList<qreal>&)),
                                        graphTool,SLOT(setValues(const QStringList&, const QList<qreal>&)));

                        connect(this,SIGNAL(graph(const QStringList&, const QStringList&, const QList<qreal>&)),
                                        graphTool,SLOT(graph(const QStringList&, const QStringList&, const QList<qreal>&)));
