...
namespace Tinkercell
{
	 ...
	 bool PythonTool::setMainWindow(MainWindow * main)
	 {
		 ...
		 if (mainWindow)
		 {
			 ...
			 appDir += tr("/../../..");
			 #endif
			 pythonInterpreter = new PythonInterpreterThread(appDir + tr("/dlls/runpy"), tr("exec"), mainWindow);
	 ...
	 void PythonTool::pluginLoaded(const QString&)
	 {
	...
	*********************/
	...
	void PythonTool::runPythonFile(const QString& filename)
	{
		if (pythonInterpreter)
		{
			...
			QString name[] = {	MainWindow::userHome() + tr("/") + PROJECTNAME + tr("/") + filename,
								MainWindow::userHome() + tr("/") + PROJECTNAME + tr("/py/") + filename,
								filename,
								QDir::currentPath() + tr("/") + filename,
