...
{
// 	ModuleTool::VisualTool::VisualTool() : GraphicalTool(tr("Module Connection Tool"))
// 	{
...
// 	bool ModuleTool::VisualTool::setMainWindow(MainWindow * main)
// 	{
// 		GraphicalTool::setMainWindow(main);
// 		if (moduleTool)
// 		{
// 			makeLinks = new QAction(tr("Set input/output"),moduleTool);
// 			makeLinks->setIcon(QIcon(tr(":/images/plug.png")));
// 			connect(makeLinks,SIGNAL(triggered()),moduleTool,SLOT(makeLinks()));
// 		}
// 		return true;
// 	}
...
// 	}
//
// 	void ModuleTool::VisualTool::selected(const QList<QGraphicsItem*>& items)
// 	{
// 		GraphicsScene * scene = currentScene();
// 		if (!scene || !moduleTool) return;
//
...
// 	}
	...
	void ModuleTool::makeLinks()
	{
		GraphicsScene * scene = currentScene();
		if (!scene) return;
		makeLinks(scene->selected());
	}

	void ModuleTool::makeLinks(const QList<QGraphicsItem*>& items)
	{
// 		graphicalTool.selected(items);
	}
...
// 	}
//
// 	void ModuleTool::VisualTool::setVisible(bool b)
// 	{
// 		if  (!b)
// 		{
// 			GraphicalTool::setVisible(false);
// 			if (mainWindow)
// 				mainWindow->contextItemsMenu.removeAction(this->makeLinks);
// 			return;
// 		}
// 		GraphicsScene * scene = currentScene();
// 		if (!scene)
...
//
// 						mainWindow->contextItemsMenu.addAction(this->makeLinks);
// 					}
// 					return;
// 				}
// 			}
...
// 	}
	...
	void ModuleTool::itemsInserted(GraphicsScene* scene, const QList<QGraphicsItem *>& insertedItems, const QList<ItemHandle*>& handles)
	{
		...
		{
			if (!(
				((node = qgraphicsitem_cast<NodeGraphicsItem*>(insertedItems[i])) &&
					(node->className == ModuleLinkerItem::class_name))
				||
				((connection = qgraphicsitem_cast<ConnectionGraphicsItem*>(insertedItems[i])) &&
		...
		for (int i=0; i < handles.size(); ++i)
		{
			...
			if (handle && handle->family() && handle->family()->isA("node"))
			{
// 				handle->graphicalTools += (&graphicalTool);
			}
		}

		NodeGraphicsItem * module = 0;

		QList<QGraphicsItem*> items, moduleConnections;
		items << insertedItems;
...
// 												QList<QUndoCommand*>()
// 												<< (new RemoveCommand(tr("remove old linker"),scene,node))
// 												<< (new InsertCommand(tr("add new linker"),scene,linker)));
...
// 					commands += new RenameCommand(tr("name changed"),scene,mergeHandles,newNames);
...
// 												QList<QUndoCommand*>()
// 												<< (new InsertCommand(tr("new modules connection"),scene,mc))
// 												<< mc->command
// 												<< (new RemoveCommand(tr("remove old connection"),scene,connection)),
...
// 						CompositeCommand * command = new CompositeCommand( tr("modules connected"),
// 									QList<QUndoCommand*>()
// 									<< (new InsertCommand(tr("modules connected"),scene,mc))
...
// 		commands += mergeCommand;
// 		commands += new RenameCommand(tr("name changed"),scene,handles,newNames);
