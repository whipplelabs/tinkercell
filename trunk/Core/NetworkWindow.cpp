...
namespace Tinkercell
{
	...
	NetworkWindow::NetworkWindow(MainWindow * main, GraphicsScene * scene) : mainWindow(main), scene(0), textEditor(0), symbolsTable(this)
	{
		...
		if (main)
		{
			...
			connect(&history, SIGNAL(indexChanged(int)), main, SIGNAL (historyChanged(int)));
	...
	NetworkWindow::NetworkWindow(MainWindow * main,TextEditor * editor) : mainWindow(main), scene(0), textEditor(0), symbolsTable(this)
	{
		...
		if (main)
		{
                       ...
                       connect(editor,SIGNAL(operationInserted(TextEditor * , OpTextItem * )),
                                    main,SIGNAL(operationInserted(TextEditor * , OpTextItem * )));

                       connect(editor,SIGNAL(connectionChanged(TextEditor * , ConnectionTextItem * , ConnectionTextItem * )),
                                    main,SIGNAL(connectionChanged(TextEditor * , ConnectionTextItem * , ConnectionTextItem * )));
