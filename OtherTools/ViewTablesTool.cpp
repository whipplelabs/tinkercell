...
namespace Tinkercell
{
        ...
        void ViewTablesTool::deselect()
	{
        ...
        void ViewTablesTool::GraphicsItem2::setVisible(bool b)
        {
            if (!tool)
            {
	...
	void ViewTablesTool::itemsInserted(GraphicsScene* , const QList<QGraphicsItem *>& , const QList<ItemHandle*>& handles)
	{
        ...
        ViewTablesTool::ViewTablesTool() : Tool(tr("View Tables"))
	{
                ...
                reader.readXml(&item,appDir + tr("/OtherTools/grid.xml"));

                item.normalize();
                item.scale(35.0/item.sceneBoundingRect().width(),35.0/item.sceneBoundingRect().height());
                graphicsItem = new GraphicsItem2(this);
                graphicsItem->setToolTip(tr("View all tables"));
...
// 	if (main->tools.contains(ViewTablesTool->name))
// 		delete ViewTablesTool;
// 	else
// 		ViewTablesTool->setMainWindow(main);
