...
// 	 DynamicLibraryMenu::GraphicalActionTool::GraphicalActionTool(const QString& name, const QPixmap& pixmap): targetAction(QIcon(pixmap),name,0)
// 	 {
...
// 	 }
//
// 	 void DynamicLibraryMenu::GraphicalActionTool::selected(const QList<QGraphicsItem*>&)
// 	 {
...
// 	 }
	 ...
	 {
// 		GraphicalActionTool * gtool = new GraphicalActionTool(functionName,icon);
...
//
// 			 QDockWidget * dockWidget = mainWindow->addDockingWindow(name, &treeWidget, Qt::BottomDockWidgetArea);
// 			 if (dockWidget)
// 			 {
...
// 			 return true;
	...
	void DynamicLibraryMenu::itemsInserted(GraphicsScene* scene, const QList<QGraphicsItem *>& , const QList<ItemHandle*>& handles)
	{
		for (int i=0; i < handles.size(); ++i)
		{
			if (handles[i])
			{
// 				for (int j=0; j < showGraphicalTool.size() && j < graphicalTools.size(); ++j)
// 					if (showGraphicalTool[j])
// 					{
// 						if (handles[i]->isA(graphicalTools[j].first) &&
// 							!handles[i]->graphicalTools.contains(graphicalTools[j].second))
//
// 							handles[i]->graphicalTools += graphicalTools[j].second;
// 					}
			}
		}
	}
...
// 			}
	...
	***********************/
    ...
    void DynamicLibraryMenu::connectTCFunctions()
	{
		...
		connect(&fToS,SIGNAL(getSelectedString(QSemaphore*,int*,const QString&,const QStringList&,int)),this,SLOT(getSelectedString(QSemaphore*,int*,const QString&,const QStringList&,int)));
	...
	void DynamicLibraryMenu::getSelectedString(QSemaphore* s,int* p,const QString& name, const QStringList& list0,int option)
	{
		if (p && mainWindow)
		{
			...
			{
				QString s = QInputDialog::getItem(mainWindow,tr("Get Text"),name,list,0,false,&ok);
				if (ok)
					(*p) = list.indexOf(s);
				else
					(*p) = -1;
	...
	int DynamicLibraryMenu::_getSelectedString(const char* title,char ** list,int i)
	{
		return fToS.getSelectedString(title,list,i);
	}
	...
	int DynamicLibraryMenu_FToS::getSelectedString(const char* c, char ** list,int i)
	{
		//qDebug() << "get item dialog";
		QSemaphore * s = new QSemaphore(1);
		int p;
		s->acquire();
		emit getSelectedString(s,&p,ConvertValue(c),ConvertValue(list),i);
		s->acquire();
		s->release();
		delete s;
		return p;
