...
static PyObject * pytc_getNames(PyObject *self, PyObject *args)
{
  ...
  for(i=0; i<N; ++i )
  {
    array[i] = isList ? (void*)((int)PyInt_AsLong( PyList_GetItem( pylist, i ) )) :
    (void*)((int)PyInt_AsLong( PyTuple_GetItem( pylist, i ) ));
  }
...
static PyObject * pytc_createInputWindow(PyObject *self, PyObject *args)
{
  ...
  for(i=0; i < rows1; ++i)
  {
    M.rownames[i] = isList1 ?
      PyString_AsString( PyList_GetItem( rownames, i ) ) :
      PyString_AsString( PyTuple_GetItem( rownames, i ) );
  }
  ...
  {
    M.values[i] = isList2 ?
      PyFloat_AsDouble( PyList_GetItem( values, i ) ) :
      PyFloat_AsDouble( PyTuple_GetItem( values, i ) );
  }

  tc_createInputWindow(M,filename,funcname, title);
...
static PyObject * pytc_addInputWindowOptions(PyObject *self, PyObject *args)
{
  ...
  for(i=0; i < rows3; ++i)
  {
    //TODO this line is really weird!!!
    coptions[i] = isList3 ?
      PyString_AsString( PyList_GetItem( options, i ) ) :
      PyString_AsString( PyTuple_GetItem( options, i ) );
    }
...
static PyObject * pytc_setPosMulti(PyObject *self, PyObject *args)
{
  ...
  if (n1 > 0 && n2 == 2)
  {
    ...
    {
      array[i] = isList1 ?
        (void*)PyInt_AsLong( PyList_GetItem( items, i ) ) :
        (void*)PyInt_AsLong( PyTuple_GetItem( items, i ) );
    }
    ...
    for(i=0; i<n2; ++i)
    {
      ...
      for (j=0; j < rows; ++j)
      {
        nums[ j*n2 + i ] = isList3 ?
          ...
          PyFloat_AsDouble( PyTuple_GetItem( item, j ) );
      }
...
static PyObject * pytc_getPos(PyObject *self, PyObject *args)
{
  ...
  {
    array[i] = isList ?
      (void*)((int)PyInt_AsLong( PyList_GetItem( pylist, i ) )) :
      (void*)((int)PyInt_AsLong( PyTuple_GetItem( pylist, i ) ));
  }
...
static PyObject * pytc_setNumericalDataMatrix(PyObject *self, PyObject *args)
{
  ...
  if (n3 > 0)
  {
    ...
    if (n1 == n3)
    {
      ...
      {
        cols[i] = isList1 ?
          PyString_AsString( PyList_GetItem( colNames, i ) ) :
          PyString_AsString( PyTuple_GetItem( colNames, i ) );
      }
    }
